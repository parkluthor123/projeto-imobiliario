...
@section('content')
    ...
    <section class="container-admin">
        ...
        <section class="form-content-area">
            ...
            <div class="form-content-wrapper">
                <div class="btn-area-estados">
                    <a href="{{ route('adm.agendamento.create') }}" title="Cadastrar">Cadastrar <i class="icon-plus"></i></a>
                </div>
                <div class="admin-table" style="position: relative">
                    ...
                    <table>
                        <div class="search-box">
                            ...
                            <input type="text" id="filter" onkeyup="getFilter(this.value, '{{ route('adm.agendamentos.ajax') }}')" placeholder="Pesquisa">
                        </div>
                        <thead>
                        <tr>
                            <th>Nome</th>
        ...
        <script>
            const titlePage = new admTitlePage("Agendamentos", "Visualize os agendamentos, clique em <i class='icon-plus'></i> para adicionar, <i class='icon-eye'></i> para visualizar e <i class='icon-trash'></i> para apagar.");
                            ...
                            Object.values(responseJson).map((data)=>(
                                table.innerHTML += `
                                   <tr>
                                       ...
                                       <td>
                                           <div class="btn-area">
                                               <a href="{{ url('/admin/agendamentos/edit/') }}/${data.id}"><i class="icon-pencil"></i> Editar</a>
                                               <button type="button" id="deleteRow" onclick="openModal('{{ url('/admin/agendamentos/delete/') }}', ${data.id}, '{{ route('adm.agendamento.all') }}')"><i class='icon-trash'></i>Apagar</button>
                                           </div>
                                       </td>
                                   </tr>
                               `
            ...
            getData("{{ route('adm.agendamento.all') }}")
        </script>
    </section>
