    ...
    <section class="container-admin">
        ...
        <section class="modal-file-adicionar-area" data-js="mensalidade-wrapper">
            <div class="modal-file-mensalidade-adicionar">
                <form action="{{ url('/admin/clientes/'.$clientes['id'].'/contrato/store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-items">
                        <div class="fileModal">
                            ...
                            <input type="file" onchange="dataFile.boleto(this)" name="contrato" id="fileSubmit">
                        </div>
                    </div>
                    <div class="btn-area">
        ...
        <script>
            const titlePage = new admTitlePage("Contratos", "Visualize os contratos, clique em <i class='icon-plus'></i> para adicionar, <i class='icon-eye'></i> para visualizar e <i class='icon-trash'></i> para apagar.");
                        ...
                        else
                        {
                            Object.values(responseJson).map((data)=>(
                               ...
                               <tr>
                                   ...
                                   <td>${data.descricao_contrato}</td>
                                   <td>
                                        <div class="btn-area">
                                            <a href="${data.contrato}" target="_blank"><i class="icon-eye"></i> Visualizar</a>
                                            <button type="button" id="deleteRow" onclick="openModal('{{ url('/admin/clientes/apagar/contrato/') }}', ${data.id}, '{{ url('/admin/clientes/editar/'.$clientes['id'].'/contratos/all') }}')"><i class='icon-trash'></i>Apagar</button>
                                        </div>
                                   </td>
            ...
            getData("{{ url('/admin/clientes/editar/'.$clientes['id'].'/contratos/all') }}")
        </script>
    </section>
